// lib/features/inspection/widgets/documents_element_card.dart
...
class DocumentsElementCard extends StatefulWidget {
  final int elementIndex;
  final DocumentsElementMeta meta;
  final DocumentsElementScore score;
  final ValueChanged<DocumentsElementScore> onChanged;

  const DocumentsElementCard({
    super.key,
  ...
  @override
  State<DocumentsElementCard> createState() => _DocumentsElementCardState();
}

class _DocumentsElementCardState extends State<DocumentsElementCard>
  ...
  bool get _isManualElement => widget.meta.key == 'aerodrome_manual';
  bool get _isUpdatesElement => widget.meta.key == 'regular_updates';
  bool get _isLicensesElement => widget.meta.key == 'licenses_validity';
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              if (_isUpdatesElement)
                                _TagBadge(
                                  label: 'Amendment',
                                  color: AppColors.acceptable,
                                ),
                              if (_isLicensesElement)
                                _TagBadge(
                                  label: 'NOTAM',
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        // تحذير الدليل
                        if (_isManualElement) ...[
                          _DocumentsSpecialWarning(
                            icon: Icons.menu_book_outlined,
                            color: const Color(0xFF1565C0),
                            bgColor: const Color(0xFFE3F2FD),
                            message:
                                'غياب الدليل = لا يمكن منح شهادة تشغيل المطار — درجة 0 تلقائياً',
                          ),
                        ...
                        if (_isUpdatesElement) ...[
                          _DocumentsSpecialWarning(
                            ...
                            message:
                                'دليل غير محدَّث منذ أكثر من سنتين = قد يحتوي معلومات خاطئة وخطيرة',
                          ),
                          const SizedBox(height: 10),
                        ],
                        // تحذير الرخص
                        if (_isLicensesElement) ...[
                          _DocumentsSpecialWarning(
                            ...
                            message:
                                'رخصة تشغيل المطار منتهية = إيقاف المطار فوراً — لا تشغيل قانوني',
                          ),
                          const SizedBox(height: 10),
                        ],

                        // المعايير الفرعية
                        _DocumentsSubCriteriaSection(
                          meta: widget.meta,
...
class _DocumentsSpecialWarning extends StatelessWidget {
  final IconData icon;
  final Color color;
  final Color bgColor;
  final String message;

  const _DocumentsSpecialWarning({
    required this.icon,
    required this.color,
...
class _DocumentsSubCriteriaSection extends StatelessWidget {
  final DocumentsElementMeta meta;
  final Map<String, bool> checked;
  final void Function(String key, bool val) onChanged;

  const _DocumentsSubCriteriaSection({
    required this.meta,
...
class _DocumentsSafetyBanner extends StatelessWidget {
  final String note;
  final int score;

  const _DocumentsSafetyBanner({required this.note, required this.score});
