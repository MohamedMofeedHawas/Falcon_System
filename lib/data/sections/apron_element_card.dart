// lib/features/inspection/widgets/apron_element_card.dart
...
class ApronElementCard extends StatefulWidget {
  final int elementIndex;
  final ApronElementMeta meta;
  final ApronElementScore score;
  final ValueChanged<ApronElementScore> onChanged;

  const ApronElementCard({
    super.key,
  ...
  @override
  State<ApronElementCard> createState() => _ApronElementCardState();
}

class _ApronElementCardState extends State<ApronElementCard>
  ...
  // عنصر مرافق دعم الطائرة قد يُصفَّر في حالة غياب نقاط الإطفاء
  bool get _isZeroRisk => widget.meta.key == 'support_facilities';

  // عنصر AGNIS/VDGS له تحذير ≥5 للمطار الليلي
  bool get _isGuidanceElement => widget.meta.key == 'guidance_systems';
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        // ── تحذير التصفير لمرافق الدعم ───────────────
                        if (_isZeroRisk) ...[
                          _ZeroScoreWarning(),
                          const SizedBox(height: 10),
                        ],

                        // ── تحذير AGNIS/VDGS الليلي ───────────────────
                        if (_isGuidanceElement) ...[
                          _GuidanceNightWarning(),
                          const SizedBox(height: 10),
                        ],

                        // ── المعايير الفرعية ──────────────────────────
                        _ApronSubCriteriaSection(
                          meta: widget.meta,
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'عدم توفر نقاط إطفاء = مخالفة خطيرة — يُضبَط التقييم على 0 مؤقتاً حتى الإصلاح',
              style: AppFonts.labelSmall.copyWith(
...
class _GuidanceNightWarning extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Container(
      width: double.infinity,
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'مطار ليلي كثيف الحركة: غياب AGNIS/VDGS = الحد الأقصى للدرجة 5 من 10',
              style: AppFonts.labelSmall.copyWith(
...
class _ApronSubCriteriaSection extends StatelessWidget {
  final ApronElementMeta meta;
  final Map<String, bool> checked;
  final void Function(String key, bool val) onChanged;

  const _ApronSubCriteriaSection({
    required this.meta,
...
class _ApronSafetyBanner extends StatelessWidget {
  final String note;
  final int score;
  const _ApronSafetyBanner({required this.note, required this.score});
